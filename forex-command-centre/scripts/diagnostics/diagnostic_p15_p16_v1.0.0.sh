...
RAW=$(curl -s "${API}/location-history?limit=5000")

if [ -z "$RAW" ]; then
    echo "ERROR: No response from ${API}/location-history"
    echo "Is the trading-state container running? Check: docker ps | grep trading-state"
    exit 1
fi
