...
(function() {
    ...
    function buildCriticalEventsSection() {
        ...
        critical.forEach(function(e, i) {
            ...
            // Use a named function call to avoid quote nesting issues
            html += '<div onclick="psbToggleEvent(\'' + eid + '\')" style="padding:6px 8px;cursor:pointer;display:flex;align-items:center;gap:8px;border-left:3px solid ' + col + ';">';
            ...
            html += '<div style="font-weight:600;font-size:0.8rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">' + (e.title || '') + '</div>';
            html += '<div style="font-size:0.72rem;color:var(--text-muted);">' + day + ' ' + time + ' AEST</div>';
            html += '</div>';
        ...
        if (hasMore) {
            ...
            html += '</div>';
            html += '<div id="psb-events-toggle" onclick="psbToggleEvents(' + rem + ')" style="padding:6px 0;text-align:center;font-size:0.76rem;color:var(--text-muted);cursor:pointer;">&#x25BC; ' + rem + ' more events</div>';
        }
        html += '</div>';
        return html;
    }

    // Helper functions for event row toggles (avoids inline quote nesting)
    window.psbToggleEvent = function(id) {
        var el = document.getElementById(id);
        if (el) el.style.display = el.style.display === 'none' ? 'block' : 'none';
    };
    window.psbToggleEvents = function(rem) {
        var o = document.getElementById('psb-events-overflow');
        var t = document.getElementById('psb-events-toggle');
        if (!o || !t) return;
