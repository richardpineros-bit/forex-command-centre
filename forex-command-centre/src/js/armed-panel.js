...
(function() {
    ...
    // v1.19.0 — Format an armed timestamp for trader visibility:
    //   aestTime  : 'HH:MM' on the same AEST day; 'DD/MM HH:MM' if armed on a prior day
    //   ageString : '42m ago' / '3h 12m ago' / '1d 4h ago'
    // Uses Australia/Sydney to auto-handle AEDT/AEST transitions.
    function formatArmedTime(timestamp) {
        ...
        if (ageMins < 1) {
            ...
            ageString = ageMins + 'm ago';
        } else {
            var h = Math.floor(ageMins / 60);
            var m = ageMins % 60;
            if (h < 24) {
                ageString = m > 0 ? h + 'h ' + m + 'm ago' : h + 'h ago';
            } else {
    ...
    function buildRow(p, emoji, tier) {
        ...
        // v1.19.0 — Fired-time row: shows AEST clock + relative age so trader
        // can locate the exact candle on the chart when reviewing alerts that
        // fired during work hours.
        var armedFt = formatArmedTime(p.timestamp);
        statusHtml += '<div class="armed-fired-time" title="Webhook arrival time at api.pineros.club" ' +
            'style="font-size:0.65rem;color:var(--text-muted);margin-top:2px;line-height:1.2;font-weight:500">' +
            'Fired ' + armedFt.aestTime + ' AEST \u00b7 ' + armedFt.ageString +
            '</div>';
