#!/usr/bin/env python3
"""
myfxbook_sentiment_scraper.py v1.1.0

Retail crowd sentiment via Myfxbook Community Outlook official public API,
plus best-effort current mid prices from Oanda /v3/pricing for trapped-crowd
analysis (Intel Hub OB Tier 2 — TODO P12).

Replaces oanda_orderbook_scraper.py (v3.0.0) — same output schema + 6 bonus
fields (position counts, volumes, average entry prices) + 1 new bonus block
("current_prices") for trapped-crowd analysis in the Intel Hub.
...
SOURCE
    ...
    GET https://api-fxtrade.oanda.com/v3/accounts/{ACCOUNT}/pricing  (best-effort)
...
OUTPUT
    ...
    contrarian_signal, label). The six bonus fields are additive. The new
    "current_prices" top-level block is also additive — existing consumers
...
CONFIG
    ...
    Oanda (env vars only — best-effort, fail-open):
    ...
    If Oanda env vars are missing or the call fails for any reason, the
    scraper still writes sentiment data without prices. The "current_prices"
    block in output JSON will simply be {} or contain a partial set.

CRON (every 4h)
    0 */4 * * * /usr/bin/python3 \\
        /mnt/user/appdata/forex-command-centre/backend/scripts/myfxbook_sentiment_scraper_v1.1.0.py \\
        --unraid

CHANGELOG
    v1.1.0  Added best-effort Oanda current-price fetch after Myfxbook logout.
            Output JSON gains a top-level "current_prices" object: {pair: mid}.
            History entries also carry "current_prices" for back-testing
            trapped-crowd -> price-move correlation later. Fail-open: any
            Oanda failure (missing creds, network, API error) leaves
            current_prices = {} with sentiment data intact. Pair mapping
            handles 6-char standard pairs plus WTICOUSD/BCOUSD overrides.
            BTCUSD / non-Oanda symbols silently skipped.
...
# --- Oanda config (env vars only — best-effort, fail-open) -------------------
...
OANDA_TIMEOUT    = 10  # tighter than Myfxbook — best-effort, don't hold up the whole run
...
# Oanda symbol overrides for FCC pair names that don't follow the
# 3+3 letter convention.
OANDA_PAIR_OVERRIDES = {
    "WTICOUSD": "WTICO_USD",
    "BCOUSD":   "BCO_USD",
}

# Explicitly skip pairs not on Oanda v20. If BTCUSD is included in a batch
# pricing request and Oanda rejects it, the WHOLE batch fails — fail-open
# would then drop all 32 other pairs. Skip it at mapping time instead.
OANDA_SKIP = {
    "BTCUSD",
}
...
# --- Oanda current price fetch (best-effort, fail-open) ----------------------
#
# This is enrichment data for trapped-crowd analysis (TODO P12 / Tier 2).
# The sentiment data is the primary value — if Oanda is unreachable, we
# still publish sentiment with current_prices = {}. The dashboard will
# simply show "—" in the Status column for affected pairs.
#
# ----------------------------------------------------------------------------

def pair_to_oanda(pair):
    """Convert FCC pair name -> Oanda v20 instrument symbol.
    Returns None if pair is not mappable or explicitly skipped
    (e.g. BTCUSD on standard accounts)."""
    if not pair:
        return None
    if pair in OANDA_SKIP:
        return None
    if pair in OANDA_PAIR_OVERRIDES:
        return OANDA_PAIR_OVERRIDES[pair]
    if len(pair) == 6:
        return pair[:3] + "_" + pair[3:]
    return None


def oanda_request(path, timeout=OANDA_TIMEOUT):
    ...
    try:
        ...
        raise RuntimeError(f"Oanda JSON decode error: {e}")


def fetch_current_prices(pairs, verbose=False):
    """
    Best-effort batch fetch of mid prices. Returns {fcc_pair: mid_price}.
    Never raises — any failure returns {} (or partial dict on per-pair issues).

    Spec (TODO P12): Sentiment is the primary value. Prices are enrichment
    for trapped-crowd analysis. Fail-open if anything goes wrong.
    """
    if not OANDA_API_KEY or not OANDA_ACCOUNT_ID:
        if verbose:
            print("[PRICE] Skipped — OANDA_API_KEY / OANDA_ACCOUNT_ID not set")
        return {}
    ...
    if not instruments:
        if verbose:
            print("[PRICE] No mappable pairs — nothing to fetch")
        return {}

    if verbose and skipped:
        print(f"[PRICE] Skipped non-Oanda pairs: {skipped}")

    path = (f"/v3/accounts/{OANDA_ACCOUNT_ID}/pricing"
            f"?instruments=" + ",".join(instruments))
    try:
        data = oanda_request(path)
    except RuntimeError as e:
        if verbose:
            print(f"[PRICE] Oanda call failed (fail-open): {e}")
        return {}
    ...
    for p in data.get("prices", []):
        ...
        if bid <= 0 or ask <= 0:
            continue
        mid = (bid + ask) / 2.0
        pair = instr_to_pair.get(instr)
        if pair:
            out[pair] = round(mid, 5)

    if verbose:
        print(f"[PRICE] Got prices for {len(out)}/{len(instruments)} mappable pairs "
              f"(skipped {len(skipped)} non-Oanda)")
    return out
...
def run_scrape(config_path, verbose=False):
    """Full scrape flow: login -> fetch -> logout -> oanda prices.
    Returns output dict."""
    ...
    # Best-effort price enrichment (fail-open) ----------------------------
    current_prices = fetch_current_prices(UTCC_PAIRS, verbose=verbose)
    ...
    return {
        ...
        "health": {
            ...
            "prices_source":         "oanda" if current_prices else "unavailable",
        },
    }
...
def main():
    ...
    print(f"[{data['last_updated']}] "
          ...
          f"{h['instruments_failed']} failed, "
          f"prices={h['prices_fetched']} ({h['prices_source']}), "
          f"history={hc}")
