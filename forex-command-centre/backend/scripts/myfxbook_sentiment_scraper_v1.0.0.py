#!/usr/bin/env python3
"""
myfxbook_sentiment_scraper.py v1.0.0
...
CRON (every 4h)
    0 */4 * * * /usr/bin/python3 \\
        /mnt/user/appdata/forex-command-centre/backend/scripts/myfxbook_sentiment_scraper_v1.0.0.py \\
        --unraid

CHANGELOG
    v1.0.0  Initial release. Full replacement for oanda_orderbook_scraper.
            33 UTCC pairs (32 direct Myfxbook names + 2 aliases: USOIL->WTICOUSD,
...
# UTCC scanner universe — canonical FCC names
UTCC_PAIRS = [
...
# First match in the list wins. Probe v1.0.0 confirmed:
#   USOIL   -> WTICOUSD   (Myfxbook also exposes XTIUSD — backup alias)
#   XBRUSD  -> BCOUSD     (Myfxbook uses XBRUSD for Brent crude)
ALIAS_MAP = {
...
def myfxbook_login(session, email, password, verbose=False):
    """Login. Returns session ID string. Raises RuntimeError on failure."""
    url = f"{API_BASE}/login.json"
    r = session.get(url, params={"email": email, "password": password}, timeout=TIMEOUT)
...
def myfxbook_get_outlook(session, myfxbook_session, verbose=False):
    """Fetch community outlook. Returns list of symbol dicts.
    Raises RuntimeError on failure."""
    url = f"{API_BASE}/get-community-outlook.json"
    r = session.get(url, params={"session": myfxbook_session}, timeout=TIMEOUT)
    if r.status_code != 200:
        raise RuntimeError(f"Outlook HTTP {r.status_code}: {r.text[:200]}")
    data = r.json()
    if data.get("error"):
...
def myfxbook_logout(session, myfxbook_session, verbose=False):
    """Best-effort logout. Never raises."""
    try:
        url = f"{API_BASE}/logout.json"
        session.get(url, params={"session": myfxbook_session}, timeout=TIMEOUT)
        if verbose:
...
def build_entry(mfb_symbol, mfb_data):
    ...
    # Normalise to 100% in case Myfxbook returns values that don't sum exactly
...
def run_scrape(config_path, verbose=False):
    ...
    session.headers.update({"User-Agent": f"FCC-MyfxbookScraper/{VERSION}"})
    ...
    try:
        ...
        mfb_session = myfxbook_login(session, email, password, verbose=verbose)
        symbols = myfxbook_get_outlook(session, mfb_session, verbose=verbose)
    finally:
        if mfb_session:
            myfxbook_logout(session, mfb_session, verbose=verbose)

    # Index Myfxbook symbols by name (uppercase for safety)
    mfb_by_name = {}
    for s in symbols:
