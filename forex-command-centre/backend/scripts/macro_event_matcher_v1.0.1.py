...
Changelog:
    v1.0.1 - Calendar path fix. v1.0.0 pointed at
             /mnt/user/appdata/trading-state/data/calendar.json which
             does not exist on this deployment. The FF calendar scraper
             writes its canonical output to
             /mnt/user/appdata/forex-command-centre/src/calendar.json
             (with a backup copy at .../data/calendar.json).
             Result in v1.0.0: matcher ran every minute for 30+ min
             logging "calendar file empty or missing" and capturing
             nothing. Fail-closed design meant no bad data shipped,
             but no good data either.
             Fix: CALENDAR_FILE constant now points at the live scraper
             output path. No other logic changes.
...
# Calendar file is written by the FF scraper to the forex-command-centre repo,
# not into trading-state/data. The scraper's DEFAULT_OUT is src/calendar.json
# with a backup at data/calendar.json (both under forex-command-centre root).
# We read the canonical src/ copy.
CALENDAR_FILE          = "/mnt/user/appdata/forex-command-centre/src/calendar.json"
